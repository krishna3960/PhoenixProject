defmodule DiscussWeb.RoomLive do
  ...
  def mount(%{"id" => room_id},_session, socket) do
    ...
    if connected?(socket) do
      DiscussWeb.Endpoint.subscribe(topic)
      DiscussWeb.Presence.track(self(), topic, username, %{})
      end
  ...
  @impl true
  def handle_event("submit_message", %{"chat" => %{"message" => message}} = params, socket) do
  message = %{uuid: UUID.uuid4(), content: message, username: socket.assigns.username}
  IO.inspect(message)
  DiscussWeb.Endpoint.broadcast(socket.assigns.topic, "new-message", message)
  ...
  def handle_info(%{event: "presence_diff", payload: %{joins: joins, leaves: leaves}}, socket) do
    ...
    user_list = DiscussWeb.Presence.list(socket.assigns.topic)
    |> Map.keys()
